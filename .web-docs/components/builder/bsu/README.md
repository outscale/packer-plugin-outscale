...
- `omi_description` (string) - The description to set for the resulting OMI(s). By default this description is empty. Both legacy JSON templates and HCL2 templates are supported. See [Packer templates](/packer/docs/templates). See [Build template data](#build-template-data) for the contextual variables available to the legacy JSON template engine.
...
- `run_tags` (object of key/value strings) - Tags to apply to the VM
  ...
  resulting OMIS unless they're duplicated in `tags`. Both legacy JSON templates and HCL2 templates are supported. See [Packer templates](/packer/docs/templates). See [Build template data](#build-template-data) for the contextual variables available to the legacy JSON template engine.
...
- `run_volume_tags` (object of key/value strings) - Tags to apply to the
  ...
  to the resulting OMIS unless they're duplicated in `tags`. Both legacy JSON templates and HCL2 templates are supported. See [Packer templates](/packer/docs/templates). See [Build template data](#build-template-data) for the contextual variables available to the legacy JSON template engine.
...
- `snapshot_tags` (object of key/value strings) - Tags to apply to snapshot.
  They will override OMIS tags if already applied to snapshot. Both legacy JSON templates and HCL2 templates are supported. See [Packer templates](/packer/docs/templates). See [Build template data](#build-template-data) for the contextual variables available to the legacy JSON template engine.
...
- `tags` (object of key/value strings) - Tags applied to the OMIS and relevant snapshots. Both legacy JSON templates and HCL2 templates are supported. See [Packer templates](/packer/docs/templates). See [Build template data](#build-template-data) for the contextual variables available to the legacy JSON template engine.
...
source "outscale-bsu" "basic-example" {
   ...
   omi_name           = "packer-osc-${formatdate("YYYY-MM-DD'T'hh-mm-ss'Z'", timestamp())}"
   omi_boot_modes     = ["uefi"]
