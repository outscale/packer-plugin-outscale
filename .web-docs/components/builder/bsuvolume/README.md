...
- `ebs_volumes` (array of block device mappings) - Add the block device
  ...
  - `tags` (map) - Tags to apply to the volume. These are retained after
    the builder completes. Both legacy JSON templates and HCL2 templates are supported. See [Packer templates](/packer/docs/templates). See [Build template data](#build-template-data) for the contextual variables available to the legacy JSON template engine.
...
- `run_tags` (object of key/value strings) - Tags to apply to the instance
  ...
  resulting OMI unless they're duplicated in `tags`. Both legacy JSON templates and HCL2 templates are supported. See [Packer templates](/packer/docs/templates). See [Build template data](#build-template-data) for the contextual variables available to the legacy JSON template engine.
